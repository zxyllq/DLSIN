...
class DDSINModel_RS(SequentialBaseModel):
    ...
    def _build_train_opt(self):
        ...
        return super(DDSINModel_RS, self)._build_train_opt()
        ...
        with tf.variable_scope("ddsin"):
                ...
                with tf.variable_scope("short_term"):
                   ...
                   # short_query_emb =   tf.concat([short_term_intention , self.target_item_embedding], -1)
                    ...
                    self.att_fea_short  = tf.concat([self.att_fea_current_pre_rnn,self.att_fea_current_pre_rnn],1)
                   ...
                   # mask_sequence = self.iterator.sequence_mask
                    ...
                    alpha_logit  = self._fcn_net(
                        tf.concat([ final_state,
                                  self.att_fea_long,
                                   self.att_fea_short    ,
                                self.target_item_embedding,tf.expand_dims(self.iterator.time_to_now[:, -1], -1)],1), hparams.alpha_fcn_layer_sizes, scope="alpha_fcn"
                    )
