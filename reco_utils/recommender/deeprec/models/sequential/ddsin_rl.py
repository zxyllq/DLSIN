...
class DDSINModel_RL(SequentialBaseModel):
    ...
    def __init__(self, hparams, iterator_creator, graph=None, seed=None):
        ...
        with self.graph.as_default():
            ...
            hparams.max_seq_length *5  ,    hparams.attention_size, 1e-08
        ) 
    ...
    def _build_train_opt(self):
        ...
        return super(DDSINModel_RL, self)._build_train_opt()
    ...
    def _build_embedding(self):
        ...
        with tf.variable_scope("embedding", initializer=self.initializer):
            ...
            self.position_embedding_lookup_long = tf.get_variable(
                ...
                shape=[self.hparams.max_seq_length*5, self.item_embedding_dim + self.cate_embedding_dim],
                dtype=tf.float32,
            )#加上position
            self.position_embedding_lookup_short = tf.get_variable(
                name="position_embedding_short",
    ...
    def _lookup_from_embedding(self):
       ...
       # tf.summary.histogram("user_short_embedding_output", self.user_short_embedding)
        ...
        self.item_session_all_embedding = tf.nn.embedding_lookup(
            self.item_lookup, self.iterator.item_session_all
        )

        self.cate_session_all_embedding = tf.nn.embedding_lookup(
            self.cate_lookup, self.iterator.item_cate_session_all
        )
####################################################################
...
####################################################################
        ...
        self.position_embedding_long = tf.nn.embedding_lookup(
            ...
            tf.tile(tf.expand_dims(tf.range(self.hparams.max_seq_length*5), 0),
                    [ tf.shape( self.iterator.item_current_session_pre )[0] , 1])
        )
        self.position_embedding_short = tf.nn.embedding_lookup(
            self.position_embedding_lookup_short,
            tf.tile(tf.expand_dims(tf.range(self.hparams.max_seq_length), 0),
                    [ tf.shape( self.iterator.item_current_session_pre  )[0] , 1])
        )
         
        self.embed_params.append(self.position_embedding_long)
        self.embed_params.append(self.position_embedding_short)
        ...
        self.item_session_all_embedding = self._dropout(
            self.item_session_all_embedding, keep_prob=self.embedding_keeps
        )
        self.cate_session_all_embedding = self._dropout(
            self.cate_session_all_embedding, keep_prob=self.embedding_keeps
        )
#######################################################################
...
#######################################################################
        ...
        self.position_embedding_short = self._dropout(
            self.position_embedding_short, keep_prob=self.embedding_keeps
        )
    ...
    def _build_seq_graph(self):
        ...
        with tf.variable_scope("ddsin"):
               
                #sequence
                sequence_input  = tf.concat(
                    [self.item_sequence_embedding, self.cate_sequence_embedding] ,  2
                )#(?,20,40)
                sessions_input  = tf.concat(
                    [self.item_session_all_embedding, self.cate_session_all_embedding] ,  2
                ) 
                ...
                mask_sessions = self.iterator.sessions_mask 
               
                real_mask_sessions = tf.cast( mask_sessions, tf.float32) 
                sessions_length = tf.reduce_sum(mask_sessions, 1)
                ...
                with tf.variable_scope("long_term"):
                   ...
                   # long_query_emb = tf.reduce_mean(sequence_pre_input, 1) 
                    ...
                    sess_sasrec_emd = self._build_sasrec( sessions_input +self.position_embedding_long ,real_mask_sessions , sessions_length ,scope='sess_long',num_heads=2,causality=False  )#long-interest公用一个sasrec
                    ...
                    sess_lstm_emd ,_ = dynamic_rnn(
                        ...
                        sequence_length= sessions_length ,
                    ...
                    att_fea_long_rnn =self._attention_fcn(long_query_emb, sess_lstm_emd ,real_mask_sessions)
                    att_fea_long_sasrec =self._attention_fcn(long_query_emb, sess_sasrec_emd,real_mask_sessions)
                ...
                with tf.variable_scope("short_term"):
                    ...
                    with  tf.variable_scope("short_term_current",reuse=tf.AUTO_REUSE):
                        ...
                        att_fea_current_pre_rnn = self._build_sasrec(current_session_pre_input  ,real_mask_current_session_pre , self.current_session_pre_length,scope='sess_short',num_heads=2,causality=False  )#long-interest公用一个sasrec
                        ...
                        att_fea_current_pre_rnn_1 = self._build_sasrec(current_session_pre_1_input  ,real_mask_current_session_pre_1 , self.current_session_pre_1_length,scope='sess_short',num_heads=2,causality=False  )#long-interest公用一个sasrec
                        ...
                        # )
                        att_fea_current_pre_rnn_2 = self._build_sasrec(current_session_pre_2_input   ,real_mask_current_session_pre_2 , self.current_session_pre_2_length,scope='sess_short',num_heads=2,causality=False  )#long-interest公用一个sasrec
