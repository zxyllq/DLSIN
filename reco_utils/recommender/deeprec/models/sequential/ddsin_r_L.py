...
class DDSINModel_r_L(SequentialBaseModel):
    ...
    def _build_train_opt(self):
        ...
        return super(DDSINModel_r_L, self)._build_train_opt()
...
#######################################################################
    ...
    def _build_seq_graph(self):
        ...
        with tf.variable_scope("ddsin"):
                ...
                #用于长期兴趣
                sequence_all_input  = tf.concat(
                    [self.item_sequence_embedding, self.cate_sequence_embedding] ,  2
                ) 
                mask_sequence_all = self.iterator.sequence_mask 
                #mask_sequence = self.iterator.sequence_mask
                real_mask_sequence_all = tf.cast( mask_sequence_all, tf.float32) 
                sequence_all_length = tf.reduce_sum(mask_sequence_all, 1)
                ...
                with tf.variable_scope("long_term"):
                   ...
                   # long_query_emb = tf.reduce_mean(sequence_pre_input, 1) 
                    ...
                    att_outputs_long = self._attention_fcn(self.user_long_embedding, sequence_all_input,real_mask_sequence_all)
                    att_fea_long = tf.reduce_sum(att_outputs_long, 1)   
                    self.att_fea_long=tf.concat([att_fea_long, att_fea_long ],1)
