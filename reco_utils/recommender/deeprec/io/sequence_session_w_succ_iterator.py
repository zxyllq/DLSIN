...
__all__ = ["SequenceSessionSuccLSIterator" ,"SequenceSessionMaskLSIterator","SequenceSessionCropLSIterator","SequenceSessionSuccExLSIterator","SequenceSessionReorderLSIterator","SequenceSessionSuccExwMaskLSIterator","SequenceSessionSuccExwReorderLSIterator,SequenceSessionSuccExwCropLSIterator"]
...
class SequenceSessionSuccExwMaskLSIterator(SequenceSessionSuccLSIterator):
    def __init__(self, hparams, graph, col_spliter="\t"):
        """Initialize an iterator. Create necessary placeholders for the model.
        ...
        Args:
            ...
            col_spliter (str): column spliter in one line.
        """
        super(SequenceSessionSuccExwMaskLSIterator, self).__init__(hparams, graph, col_spliter)
        self.augment_rate =hparams.augment_rate
        with self.graph.as_default():
            self.item_current_session_pre_1 = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="item_current_session_pre_1"
       ...
       label_list,
        ...
        if batch_num_ngs:
            ...
            for i in range(instance_cnt):
                ...
                item_current_session_pre_1_r =  [item_current_session_pre_batch[i][index] for index in range(current_session_pre_length)  if index not in replace_index_1] +[item_current_session_succ_batch[i][index] for index in range(current_session_succ_length) if index  in replace_index_2]
                item_cate_current_session_pre_1_r =  [item_cate_current_session_pre_batch[i][index] for index in range(current_session_pre_length) if index not in replace_index_1] +[item_cate_current_session_succ_batch[i][index] for index in range(current_session_succ_length) if index  in replace_index_2]

                item_current_session_pre_2_r=  [item_current_session_succ_batch[i][index] for index in range(current_session_succ_length) if index not in replace_index_2] +[item_current_session_pre_batch[i][index] for index in range(current_session_pre_length) if index  in replace_index_1]
                item_cate_current_session_pre_2_r =  [item_cate_current_session_succ_batch[i][index] for index in range(current_session_succ_length) if index not in replace_index_2]+[item_cate_current_session_pre_batch[i][index] for index in range(current_session_pre_length) if index  in replace_index_1]
                
                current_session_pre_1_length = len(item_current_session_pre_1_r )
                current_session_pre_2_length = len(item_current_session_pre_2_r)
                ##############################mask#################################
                remove_index_1 = random.sample(list(range(current_session_pre_1_length )),k=int(np.ceil(self.augment_rate*current_session_pre_1_length)))
                remove_index_2 = random.sample(list(range(current_session_pre_2_length)),k=int(np.ceil(self.augment_rate*current_session_pre_2_length)))


                item_current_session_pre_1 =  [ item_current_session_pre_1_r[index] if index not in remove_index_1 else 0  for index in range(current_session_pre_1_length )   ] 
                item_cate_current_session_pre_1 =  [item_cate_current_session_pre_1_r[index]  if index not in remove_index_1 else 0 for index in range(current_session_pre_1_length)]

                item_current_session_pre_2=  [item_current_session_pre_2_r[index] if index not in remove_index_2 else 0 for index in range(current_session_pre_2_length) ]
                item_cate_current_session_pre_2 =  [item_cate_current_session_pre_2_r[index] if index not in remove_index_2 else 0 for index in range(current_session_pre_2_length)  ]
...
class SequenceSessionSuccExwCropLSIterator(SequenceSessionSuccLSIterator):
    def __init__(self, hparams, graph, col_spliter="\t"):
        """Initialize an iterator. Create necessary placeholders for the model.
        ...
        Args:
            ...
            col_spliter (str): column spliter in one line.
        """
        super(SequenceSessionSuccExwCropLSIterator, self).__init__(hparams, graph, col_spliter)
        self.augment_rate =hparams.augment_rate
        with self.graph.as_default():
            self.item_current_session_pre_1 = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="item_current_session_pre_1"
       ...
       label_list,
        ...
        if batch_num_ngs:
            ...
            for i in range(instance_cnt):
                ...
                current_session_pre_2_length = len(item_current_session_pre_2_r)
                #############################crop##################################
                # remove_index_1 = random.sample(list(range(item_current_session_pre_1_r )),k=int(np.ceil(self.augment_rate*current_session_pre_1_length)))
                # remove_index_2 = random.sample(list(range(item_current_session_pre_2_r)),k=int(np.ceil(self.augment_rate*current_session_pre_2_length)))
                ...
                #current_session_pre_length = len(item_current_session_pre_batch[i])
                #data AUgmentation
                if current_session_pre_1_length >2 and current_session_pre_2_length >2 :
                    
                    crop_length_1 = int(np.ceil(self.augment_rate*current_session_pre_1_length))
                    crop_length_2 = int(np.ceil(self.augment_rate*current_session_pre_2_length))

                    start_index_1 = random.sample(list(range(current_session_pre_1_length-crop_length_1+1)) ,k=1)
                    start_index_2 = random.sample(list(range(current_session_pre_2_length-crop_length_2+1)),k=1 )
                    
                    item_current_session_pre_1 = item_current_session_pre_1_r.copy()
                    del item_current_session_pre_1[start_index_1[0]:start_index_1[0]+crop_length_1]#包括起始位
                    item_cate_current_session_pre_1 = item_cate_current_session_pre_1_r.copy()
                    del item_cate_current_session_pre_1[start_index_1[0]:start_index_1[0]+crop_length_1]

                    item_current_session_pre_2 = item_current_session_pre_2_r.copy()
                    del item_current_session_pre_2[start_index_2[0]:start_index_2[0]+crop_length_2]

                    item_cate_current_session_pre_2 =  item_cate_current_session_pre_2_r.copy()
                    del item_cate_current_session_pre_2[start_index_2[0]:start_index_2[0]+crop_length_2]
                    ...
                    item_current_session_pre_1 =  item_current_session_pre_1_r 
                    item_cate_current_session_pre_1 =  item_cate_current_session_pre_1_r  

                    item_current_session_pre_2=   item_current_session_pre_2_r
                    item_cate_current_session_pre_2 =  item_cate_current_session_pre_2_r 
...
class SequenceSessionSuccExwReorderLSIterator(SequenceSessionSuccLSIterator):
    def __init__(self, hparams, graph, col_spliter="\t"):
        """Initialize an iterator. Create necessary placeholders for the model.
        ...
        Args:
            ...
            col_spliter (str): column spliter in one line.
        """
        super(SequenceSessionSuccExwReorderLSIterator, self).__init__(hparams, graph, col_spliter)
        self.augment_rate =hparams.augment_rate
        with self.graph.as_default():
            self.item_current_session_pre_1 = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="item_current_session_pre_1"
        ...
        if batch_num_ngs:
            ...
            for i in range(instance_cnt):
                ...
                if current_session_pre_1_length >2 and current_session_pre_2_length >2 :
                    ...
                    current_session_pre_1 =np.array([ item_current_session_pre_1_r.copy(),  item_cate_current_session_pre_1_r.copy()])
                    current_session_pre_1 = np.transpose(current_session_pre_1)
                    np.random.shuffle(current_session_pre_1[start_index_1[0]:start_index_1[0]+crop_length_1])
                    current_session_pre_1 = np.transpose(current_session_pre_1)
                    ...
                    current_session_pre_2 =np.array([ item_current_session_pre_2_r.copy(),  item_cate_current_session_pre_2_r.copy()])
                    current_session_pre_2 = np.transpose(current_session_pre_2)
                    np.random.shuffle(current_session_pre_2[start_index_2[0]:start_index_2[0]+crop_length_2])
                    current_session_pre_2 = np.transpose(current_session_pre_2)
