...
__all__ = ["SessionLSIterator" ]

class SessionLSIterator(BaseIterator):
       ...
       # self.time_unit = hparams.time_unit
        ...
        with self.graph.as_default():
            ...
            self.item_session_succ = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="item_session_succ"
            )
            self.item_cate_session_succ = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="item_cate_session_succ"
            )
            self.session_succ_mask = tf.placeholder(
                tf.int32, [None, self.max_seq_length], name="session_succ_mask"
            )
    ...
    def _convert_data(
        ...
        if batch_num_ngs:
            ...
            for i in range(instance_cnt):
                ...
                session_4_length =valid_sess_batch[i][-4]
