...
__all__ = ["SessionOriginIterator" ]

class SessionOriginIterator(BaseIterator):
    def __init__(self, hparams, graph, col_spliter="\t"):
       ...
       # self.time_unit = hparams.time_unit
        ...
        with self.graph.as_default():
            ...
            )#[ -3,-2,-1,0 ]这里的max_session_count应该为0

            self.item_session_0 = tf.placeholder(
                tf.int32, [None, self.max_seq_length*2], name="item_session_0"
            )
            self.item_cate_session_0 = tf.placeholder(
                tf.int32, [None, self.max_seq_length*2], name="item_cate_session_0"
            )
            self.session_0_mask = tf.placeholder(
                tf.int32, [None, self.max_seq_length*2], name="session_0_mask"
            )
            ...
            #successor
            self.item_session_succ = tf.placeholder(
                tf.int32, [None, self.max_seq_length*2], name="item_session_succ"
            )
            self.item_cate_session_succ = tf.placeholder(
                tf.int32, [None, self.max_seq_length*2], name="item_cate_session_succ"
            )
            self.session_succ_mask = tf.placeholder(
                tf.int32, [None, self.max_seq_length*2], name="session_succ_mask"
            )
    ...
    def parser_one_line(self, line):
        ...
        valid_sess.pop(0)#session_4暂时不需要
        precessor_length =len(item_session_0)  #session_0的长度当时没能处理进去
        
        succssor_length= len(item_session_succ)#有可能为[0]
        if  succssor_length ==1 and  item_session_succ[0]==0:
    ...
    def load_data_from_file(self, infile, batch_num_ngs=0, min_seq_length=1):
        ...
        item_cate_list = []
        behavior_list =[]
        precessor_length_list=[]
        succssor_length_list=[]
        item_session_succ_batch = []
        item_cate_session_succ_batch = []
        ...
        for line in lines:
            ...
            precessor_length_list.append(precessor_length )
            succssor_length_list.append(succssor_length)
            item_session_succ_batch.append(item_session_succ)
            item_cate_session_succ_batch.append(item_cate_session_succ)
    ...
    def _convert_data(
        ...
        if batch_num_ngs:
            ...
            item_session_succ_batch_all = np.zeros(
                (instance_cnt * (batch_num_ngs + 1), max_seq_length*2 )
            ).astype("int32")
            item_cate_session_succ_batch_all = np.zeros(
                (instance_cnt * (batch_num_ngs + 1), max_seq_length*2 )
            ).astype("int32")
            ...
            mask_succ = np.zeros(
                (instance_cnt * (1 + batch_num_ngs), max_seq_length*2 )
            ).astype("float32")
            mask_0 =np.zeros(
                (instance_cnt * (1 + batch_num_ngs), max_seq_length *2)
            ).astype("float32")
            ...
            for i in range(instance_cnt):
                ...
                #session_0 是一定存在的
                session_0_length =precessor_length_list[i] 
                ...
                #session_succ不一定存在
                session_succ_length= succssor_length_list[i]
                if session_succ_length>0:
                    for index in range(batch_num_ngs + 1):
                        item_session_succ_batch_all[
                            i * (batch_num_ngs + 1) + index, :session_succ_length
                        ] = np.asarray(item_session_succ_batch[i] , dtype=np.int32)
                        item_cate_session_succ_batch_all[
                            i * (batch_num_ngs + 1) + index, :session_succ_length
                        ] = np.asarray(
                            item_cate_session_succ_batch[i] , dtype=np.int32
                        )
                        mask_succ[i * (batch_num_ngs + 1) + index, :session_succ_length] = 1.0

                #3个长期序列，也不一定存在

                sess_count= 1  
                session_1_length =valid_sess_batch[i][-1]
                ...
                session_3_length =valid_sess_batch[i][-3]
            ...
            res["item_session_succ"] = item_session_succ_batch_all
            res["item_cate_session_succ"] = item_cate_session_succ_batch_all
            res["session_succ_mask"] =mask_succ
    ...
    def gen_feed_dict(self, data_dict):
        ...
        feed_dict = {
            ...
            self.item_session_succ: data_dict["item_session_succ"],
            self.item_cate_session_succ: data_dict["item_cate_session_succ"],
            self.session_succ_mask : data_dict["session_succ_mask"],
