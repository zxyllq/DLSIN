...
__all__ = ["AllSessionSuccLSIterator"]

class AllSessionSuccLSIterator(BaseIterator):
    def __init__(self, hparams, graph, col_spliter="\t"):
       ...
       # self.time_unit = hparams.time_unit
        ...
        with self.graph.as_default():
            ...
            self.item_session_all = tf.placeholder(
                tf.int32, [None,self.max_seq_length*self.max_session_count], name="item_session_all"
            )
            self.item_cate_session_all = tf.placeholder(
                tf.int32, [None, self.max_seq_length*self.max_session_count], name="item_cate_session_all"
            )
            self.sessions_mask = tf.placeholder(
                tf.int32, [None, self.max_seq_length*self.max_session_count], name="sessions_mask"
            )
    ...
    def parser_one_line(self, line):
       ...
       # index = float(words[1])#用于记录loss的变化
        ...
        if len(cate_session_4)==1 and cate_session_4[0]==0:
            item_session_4 =[]
            cate_session_4 =[]
        ...
        if len(cate_session_3)==1 and cate_session_3[0]==0:
            item_session_3 =[]
            cate_session_3 =[]
        ...
        if len(cate_session_2)==1 and cate_session_2[0]==0:
            item_session_2 =[]
            cate_session_2 =[]
        ...
        if len(cate_session_1)==1 and cate_session_1[0]==0:
            item_session_1 =[]
            cate_session_1 =[]
        ...
        if len(cate_session_0)==1 and cate_session_0[0]==0:
            item_session_0 =[]
            cate_session_0 =[]

        item_session_all = item_session_4+ item_session_3+item_session_2+ item_session_1 + item_session_0
        cate_session_all  = cate_session_4+ cate_session_3 + cate_session_2 + cate_session_1 + cate_session_0

        #过滤0后,至少保留一个
        #item_session_all =  list(filter(lambda x: x != 0, item_session_all_concat  ))
       # cate_session_all =   list(filter(lambda x: x != 0, cate_session_all_concat   ))
        if len(item_session_all )==0:
            item_session_all=[0]
            cate_session_all=[0]
    ...
    def load_data_from_file(self, infile, batch_num_ngs=0, min_seq_length=1):
        ...
        item_session_all_batch =[]
        item_cate_session_all_batch =[]
        ...
        for line in lines:
            ...
            item_session_all_batch.append(item_session_all)
            item_cate_session_all_batch.append(item_cate_session_all)
    ...
    def _convert_data(
       ...
       label_list,
        ...
        if batch_num_ngs:
            ...
            item_session_all_batch_all = np.zeros(
                (instance_cnt * (batch_num_ngs + 1), max_seq_length*max_session_count )
            ).astype("int32")
            item_cate_session_all_batch_all = np.zeros(
                (instance_cnt * (batch_num_ngs + 1), max_seq_length*max_session_count)
            ).astype("int32") 
            ...
            for i in range(instance_cnt):
                ...
                sessions_length = len(item_session_all_batch[i])
                for index in range(batch_num_ngs + 1):
                    ...
                    item_session_all_batch_all[
                        i * (batch_num_ngs + 1) + index, :sessions_length
                    ] = np.asarray(item_session_all_batch[i] , dtype=np.int32)
                    
                    item_cate_session_all_batch_all[
                        i * (batch_num_ngs + 1) + index, :sessions_length
                    ] = np.asarray(
                        item_cate_session_all_batch[i] , dtype=np.int32
                    )
                    mask_sessions[i * (batch_num_ngs + 1) + index, :sessions_length] = 1.0
            ...
            res["item_session_all"] = item_session_all_batch_all
            res["item_cate_session_all"] = item_cate_session_all_batch_all
            ...
            res["sessions_mask"] =mask_sessions
            ...
            for i in range(instance_cnt):
                ...
                item_session_all_batch_all[i, :sessions_length] = item_session_all_batch[i] 
                item_cate_session_all_batch_all[i, :sessions_length] = item_cate_session_all_batch[i] 

                mask_sequence[i, :sequence_length] = 1.0
                mask_sessions[i, :sessions_length] = 1.0
    ...
    def gen_feed_dict(self, data_dict):
        ...
        feed_dict = {
            ...
            self.item_session_all: data_dict["item_session_all"],
            self.item_cate_session_all: data_dict["item_cate_session_all"],
            ...
            self.sessions_mask: data_dict["sessions_mask"],
