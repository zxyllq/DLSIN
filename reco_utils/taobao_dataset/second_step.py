...
def current_session_succ_process(user_interactions,target_session_state , max_session_length,session_state  ):
    ...
    if current_session_succ.empty ==True :
        ...
        current_session_behavior_succ =  list(reversed(current_session_succ["behavior_type"].to_list()[:max_session_length]))
        current_session_session_state_succ = current_session_succ[ session_state ].to_list()[:max_session_length]
    return current_session_iid_succ,current_session_cid_succ,current_session_behavior_succ ,current_session_session_state_succ

def  current_session_pre_process(pre_sequence_df,target_session_state , max_session_length,session_state  ):
    #1.先将其全部转为list
    pre_sequence = {'iid':[],"cid":[],"behavior_type":[],session_state:[]}
     #注意 0，1，2，...,t-1,t 
    ...
    current_session_behavior_pre=pre_sequence["behavior_type"][-tail_length:][-max_session_length:]
    current_session_session_state_pre=pre_sequence[session_state][-tail_length:][-max_session_length:]

    #对pre_sequence截断
    pre_sequence['iid']=pre_sequence['iid'][:-tail_length]
    pre_sequence["cid"]=pre_sequence["cid"][:-tail_length]
    pre_sequence["behavior_type"] =pre_sequence["behavior_type"] [:-tail_length]
    pre_sequence[ session_state ] =pre_sequence[ session_state ] [:-tail_length]

    return pre_sequence,current_session_iid_pre,current_session_cid_pre,current_session_behavior_pre,current_session_session_state_pre

def Long_term_session_process(pre_sequence ,max_sess_num,max_session_length,current_session_state_pre,session_state,itemdict,catedict) :
    LT_session={}
    valid_sess=[]
    bak_session_state = current_session_state_pre-1
    for sess in range(1,max_sess_num):
        LT_session['LT_session'+str(-sess)]={'iid':[],"cid":[],"behavior_type":[], session_state :[]}
        #1. pre为[]
        if len(pre_sequence[ session_state ])==0:
            ...
            LT_session['LT_session'+str(-sess)]['behavior_type']=[0]
            LT_session['LT_session'+str(-sess)][ session_state ]=[0]
            valid_sess.append(0)
            continue
        #2. pre 不为空
        #2.1 选取bak_session_state作为备选state
        #2.2 统计备选state的数目
        tail_length = pre_sequence[ session_state ].count(bak_session_state)
        #2.3 在长期兴趣这里，对于长度小于等于2的，还是删除的比较好
       
        # 2.3.1 session_length =<2
        cumsum_drop =0
 
        while tail_length <=2 and int(bak_session_state) >1:
            #继续往前找
        ...
        if  cumsum_drop >0:
            ...
            pre_sequence['behavior_type']= pre_sequence['behavior_type'][:-cumsum_drop]
            pre_sequence[  session_state ]=pre_sequence[ session_state ][:-cumsum_drop]
        #正式录入长期session
        #对于长期session的较长序列，只取最后session_num个交互恐有不妥，可以采取随机起始位截断
        if tail_length >max_session_length: 
            random_position=random.randint(0, tail_length-max_session_length)
        else:
            random_position=0
        LT_session['LT_session'+str(-sess)]['iid'] = pre_sequence['iid'][-tail_length:][random_position:random_position+max_session_length]
        LT_session['LT_session'+str(-sess)]['cid']= pre_sequence['cid'][-tail_length:][random_position:random_position+max_session_length]
        LT_session['LT_session'+str(-sess)]['behavior_type'] =  pre_sequence['behavior_type'][-tail_length:][random_position:random_position+max_session_length]
        LT_session['LT_session'+str(-sess)][ session_state ] = pre_sequence[ session_state ][-tail_length:][random_position:random_position+max_session_length]
        #如果不为0，一定要记得reid
...
def session_LS_dataset_process_1(batch_num,reviews_batch,  user_batch, session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval):
    ...
    for user in tqdm(user_batch):
        ...
        for target_row in  user_interactions.index :
            ...
            current_session_iid_succ,current_session_cid_succ,current_session_behavior_succ ,current_session_session_state_succ =current_session_succ_process(user_interactions[user_interactions["state"]== target_state].loc[target_row+1:] ,target_session_state , max_session_length,session_state )
            ...
            if   pre_sequence.empty ==True :
                #无pre，那么就结束本次循环进入下一个target             
                continue
            #如果存在pre_sequence,先去取short-term current session pre,后面再一个一个取long-term sessions
            # short-term current session pre 哪怕为1 也可以取到，所以一定不为空，
            # 但Long-term sessions的长度<=2 的尽量不要取
          
            #这里的pre_sequence已经是字典了，不再是dataframe
            pre_sequence,current_session_iid_pre,current_session_cid_pre,current_session_behavior_pre,current_session_session_state_pre =current_session_pre_process(pre_sequence,target_session_state , max_session_length,session_state  )
            ...
            # Long-term_session 不规定至少有一个长期 session，另外对session——length有要求
            LT_session,valid_LT_sess =Long_term_session_process(pre_sequence ,max_sess_num,max_session_length,current_session_session_state_pre[0],session_state,itemdict,catedict) 
            # 4.4 写入文件
            ...
            for sess in range(1,max_sess_num):
                LT_all = LT_all  +str(LT_session['LT_session'+str(-sess)]['iid']) +"\t"\
                           +str(LT_session['LT_session'+str(-sess)]['cid']) +"\t"\
                            +str(LT_session['LT_session'+str(-sess)]['behavior_type']) +"\t"\
            ...
            fo.write(str(ref_index )#将train/valid/test 的index与 原reviews_file对齐
                        ...
                        + "\t"
                        + str(current_session_behavior_pre)
                        + "\t"
                        + str(current_session_session_state_pre )
...
def align_negtive_sample_method(sequence_data_file,session_LS_data_file,valid_negtive_sample,test_negtive_sample):
    ...
    #f_all_test = open(os.path.join(session_LS_data_file , r'all_test_data')+'_'+str(batch_num), "w")
    #1.确定采样index
    index = pd.read_csv(os.path.join(sequence_data_file , r'sample_valid_data'),sep='\t',header=None)[1].unique()
    #2.align 降采样
    all_valid_data =pd.read_csv(os.path.join(session_LS_data_file, r'all_valid_data'),sep='\t',header=None)

    all_valid_data = all_valid_data[all_valid_data[0].isin(index)]
    #3. align 负样本
    positive_and_negtive_iid = pd.read_csv(os.path.join(sequence_data_file , r'sample_valid_data'),sep='\t',header=None)[3]
    positive_and_negtive_cid = pd.read_csv(os.path.join(sequence_data_file , r'sample_valid_data'),sep='\t',header=None)[4]

    negtive_sample_valid_data = pd.DataFrame(np.repeat(all_valid_data.values,valid_negtive_sample+1 ,axis=0))
    negtive_sample_valid_data[2]=positive_and_negtive_iid 
    negtive_sample_valid_data[3]=positive_and_negtive_cid
    ...
    index = pd.read_csv(os.path.join(sequence_data_file , r'sample_test_data'),sep='\t',header=None)[1].unique()
    #2.align 降采样
    all_test_data =pd.read_csv(os.path.join(session_LS_data_file, r'all_test_data'),sep='\t',header=None)

    all_test_data = all_test_data[all_test_data[0].isin(index)]
    #3. align 负样本
    positive_and_negtive_iid = pd.read_csv(os.path.join(sequence_data_file , r'sample_test_data'),sep='\t',header=None)[3]
    positive_and_negtive_cid = pd.read_csv(os.path.join(sequence_data_file , r'sample_test_data'),sep='\t',header=None)[4]

    negtive_sample_test_data = pd.DataFrame(np.repeat(all_test_data.values,test_negtive_sample+1 ,axis=0))
    negtive_sample_test_data[2]=positive_and_negtive_iid 
    negtive_sample_test_data[3]=positive_and_negtive_cid
...
def session_LS_make(reviews,cpu_num, session_LS_data_file,sequence_data_file, max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file ,session_interval ,valid_negtive_sample,test_negtive_sample ):
    ...
    for i in range(0, cpu_num):
       ...
       # session_LS_dataset_process_1(i,reviews_batch, user_batch,  session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval )
        process.append(Process(target = session_LS_dataset_process_1,args =(i,reviews_batch, user_batch,  session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval) )  ) 
...
def session_make( session_LS_data_file,session_data_file ):
    train_data = pd.read_csv(os.path.join(session_LS_data_file, r'train_data'),sep='\t',header=None)
    train_data = train_data.drop(columns=[6,7,8,9])
    train_data.to_csv(os.path.join(session_data_file, r'train_data') ,sep='\t',header=None,index=False)   
     
    all_valid_data = pd.read_csv(os.path.join(session_LS_data_file, r'all_valid_data'),sep='\t',header=None)
    all_valid_data = all_valid_data.drop(columns=[6,7,8,9])
    all_valid_data.to_csv(os.path.join(session_data_file, r'all_valid_data') ,sep='\t',header=None,index=False)   
    all_test_data = pd.read_csv(os.path.join(session_LS_data_file, r'all_test_data'),sep='\t',header=None)
    all_test_data = all_test_data.drop(columns=[6,7,8,9])
    all_test_data.to_csv(os.path.join(session_data_file, r'all_test_data') ,sep='\t',header=None,index=False)   
    
    sample_valid_data = pd.read_csv(os.path.join(session_LS_data_file, r'sample_valid_data'),sep='\t',header=None)
    sample_valid_data = sample_valid_data.drop(columns=[6,7,8,9])
    sample_valid_data.to_csv(os.path.join(session_data_file, r'sample_valid_data') ,sep='\t',header=None,index=False)   
    sample_test_data = pd.read_csv(os.path.join(session_LS_data_file, r'sample_test_data'),sep='\t',header=None)
    sample_test_data = sample_test_data.drop(columns=[6,7,8,9])
    sample_test_data.to_csv(os.path.join(session_data_file, r'sample_test_data') ,sep='\t',header=None,index=False) 


    # shutil.copy( os.path.join(session_LS_data_file, r'sample_valid_data'), os.path.join(session_data_file, r'sample_valid_data'))
    # shutil.copy( os.path.join(session_LS_data_file, r'sample_test_data'), os.path.join(session_data_file, r'sample_test_data'))
...
max_sess_num = 5
session_LS_data_file =  os.path.join(reviews_path,   "session_LS",session_interval )
...
sequence_data_file =  os.path.join(reviews_path,   "sequence" )
...
session_make( session_LS_data_file,session_data_file )
