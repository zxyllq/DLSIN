    ...
    for user in tqdm(user_batch):
        ...
        for target_row in  user_interactions.index :
           ...
           # pre_sequence =  user_interactions.loc[:target_row-1].drop_duplicates(subset=['uid', 'iid'],keep='last')[-max_session_length*max_sess_num :] 
            ...
            pre_sequence =  user_interactions.loc[:target_row-1].drop_duplicates(subset=['uid', 'iid'])[-max_session_length*max_sess_num :] 
            if   pre_sequence.empty ==True :
                #无pre，那么就结束本次循环进入下一个target             
                continue
...
session_interval= '720' 
max_sequence_length=50
max_session_length =10
max_sess_num = 5
session_LS_data_file_clean =  os.path.join(reviews_path,   "session_LS",session_interval+'_clean_w_suc_r' )
os.makedirs(session_LS_data_file_clean, mode = 0o777, exist_ok = True) 
 
# sequence_data_file_clean =  os.path.join(reviews_path,   "sequence"+'_clean_r' )
# os.makedirs(sequence_data_file_clean, mode = 0o777, exist_ok = True) 

# session_LS_data_file_noise =  os.path.join(reviews_path,   "session_LS",session_interval+'_noise_'+str(noise_rate)+'_w_suc_r' )
# os.makedirs(session_LS_data_file_noise, mode = 0o777, exist_ok = True) 
 
# sequence_data_file_noise =  os.path.join(reviews_path,   "sequence"+'_noise_' +str(noise_rate)+'_r')
