...
def session_LS_dataset_process_3(batch_num,reviews_batch,  user_batch, session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval):
    ...
    for user in tqdm(user_batch):
        ...
        for target_row in  user_interactions.index :
            ...
            #pre_sequence
            #先把pre_sequence 全部取到
            pre_sequence = user_interactions.loc[:target_row-1][-max_session_length*max_sess_num :] 
            if   pre_sequence.empty ==True :
                #无pre，那么就结束本次循环进入下一个target             
                continue
...
def session_LS_make(reviews,cpu_num, session_LS_data_file,sequence_data_file, max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file ,session_interval ,valid_negtive_sample,test_negtive_sample ):
    ...
    for i in range(0, cpu_num):
        ...
        process.append(Process(target = session_LS_dataset_process_3,args =(i,reviews_batch, user_batch,  session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval) )  ) 
