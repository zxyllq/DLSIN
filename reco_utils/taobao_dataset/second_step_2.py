...
def session_LS_dataset_process_2(batch_num,reviews_batch,  user_batch, session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval):
    ...
    for user in tqdm(user_batch):
        ...
        for target_row in  user_interactions.index :
            ...
            pre_sequence,current_session_iid_pre,current_session_cid_pre,current_session_behavior_pre,current_session_session_state_pre =current_session_pre_process(pre_sequence,target_session_state , max_session_length*2,session_state  )
...
def session_LS_make(reviews,cpu_num, session_LS_data_file,sequence_data_file, max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file ,session_interval ,valid_negtive_sample,test_negtive_sample ):
    ...
    for i in range(0, cpu_num):
        ...
        #session_LS_dataset_process_2(i,reviews_batch, user_batch,  session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval )
        process.append(Process(target = session_LS_dataset_process_2,args =(i,reviews_batch, user_batch,  session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval) )  ) 
...
def session_trunk(dataframe ,max_session_length):
    for row in tqdm(range(0,len(dataframe))):
        if len(dataframe[10].iloc[row]) >10:
            dataframe[10].iloc[row]= dataframe[10].iloc[row].strip("[").strip("]").split(", ")[-max_session_length:]
            dataframe[11].iloc[row]= dataframe[11].iloc[row].strip("[").strip("]").split(", ")[-max_session_length:]
            dataframe[12].iloc[row]= dataframe[12].iloc[row].strip("[").strip("]").split(", ")[-max_session_length:]
            dataframe[13].iloc[row]= dataframe[13].iloc[row].strip("[").strip("]").split(", ")[-max_session_length:]
    return dataframe

def session_make( session_LS_data_file,session_data_file,max_session_length ):
   ...
   # train_data = session_trunk(train_data   ,max_session_length)
   ...
   # all_valid_data = session_trunk(all_valid_data   ,max_session_length)
    ...
    #all_test_data = session_trunk(all_test_data   ,max_session_length)
    ...
    #sample_valid_data = session_trunk(sample_valid_data   ,max_session_length)
   ...
   # sample_test_data = session_trunk( sample_test_data  ,max_session_length)
