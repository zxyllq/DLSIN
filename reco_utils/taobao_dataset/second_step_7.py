...
def session_LS_make(reviews,cpu_num, session_LS_data_file,  max_session_length,max_sess_num,   session_interval ,valid_negtive_sample,test_negtive_sample,sample_probability ):
    ...
    for i in range(0, cpu_num):
        ...
        #session_LS_dataset_process_3(i,reviews_batch, user_batch,  session_LS_data_file,max_session_length,max_sess_num, user_vocab_file,item_vocab_file,cate_vocab_file,session_interval ,sample_probability)
...
def change_ref_index2noise_label( data_file , noise_index):
    train_file =  pd.read_csv( os.path.join(data_file,r'train_data'), sep='\t' ,header=None )
    train_file.loc[train_file[1].isin(noise_index),[1]]=0#noise
    train_file.loc[train_file[1]>0,[1]]=1#original intreaction   

    train_file.to_csv( os.path.join(data_file,r'train_data'),  sep='\t', header=None, index=False)

    sample_valid_file =  pd.read_csv( os.path.join(data_file,r'sample_valid_data'), sep='\t' ,header=None )
    sample_valid_file.loc[sample_valid_file[1].isin(noise_index),[1]]=0#noise
    sample_valid_file.loc[sample_valid_file[1]>0,[1]]=1#original intreaction   

    sample_valid_file.to_csv( os.path.join(data_file,r'sample_valid_data'),  sep='\t', header=None, index=False)
...
max_sess_num = 5
session_LS_data_file_clean =  os.path.join(reviews_path,   "session_LS",session_interval+'_clean' )
...
session_LS_data_file_noise =  os.path.join(reviews_path,   "session_LS",session_interval+'_noise_'+str(noise_rate) )
...
change_ref_index2noise_label( sequence_data_file_noise,sequence_noise_index)
...
change_ref_index2noise_label(session_LS_data_file_noise,session_noise_index)
